#ifndef CONSTANTS_H
#define CONSTANTS_H

inline const size_t TILE_SIZE = 22;
inline const size_t BIG_DOT_POINTS = 20;
inline const size_t SMALL_DOT_POINTS = 10;
inline const float GHOST_COUNTER = 20;
inline const char* HUD_FONT = "freefont-ttf\\sfd\\FreeMono.ttf";
inline const float AVATAR_SPEED = 120.f;
inline const float GHOST_SPEED = 30.f;
inline const int AVATAR_START_TILE_X = 13;
inline const int AVATAR_START_TILE_Y = 22;
inline const int GHOST_START_TILE_X = 13;
inline const int GHOST_START_TILE_Y = 13;

#endif // CONSTANTS_H